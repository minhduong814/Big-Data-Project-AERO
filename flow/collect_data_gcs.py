...
def collect_data_to_gcs(year, month, bucket_name):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with open(zip_path, "wb") as f:
                for chunk in download_response.iter_content(chunk_size=1024):
                    if chunk:
                        f.write(chunk)
            import logging
            logger = logging.getLogger("flow.collect_data_gcs")
            ...
            if not csv_files:
                logger.warning("⚠️ No CSV found in downloaded archive.")
                return
            ...
            logger.info(f"✅ Renamed {csv_files[0]} → {year}_{month:02d}.csv")
            ...
            if bucket is None:
                ...
                logger.info(f"✅ Created bucket {bucket_name} in {bucket.location}")
            else:
                logger.info(f"🌐 Using existing bucket: {bucket.name}")
            ...
            logger.info(f"☁️ Uploaded to: gs://{bucket_name}/{blob_path}")
